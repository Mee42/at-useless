...
fun main() {
    System.err.println("==== Started @useless Server. Version: ${Params.version} ====")
    System.err.flush()
    if(Params.debugMode) {
        println("debug mode ON. URL: $SERVER_URL")
        System.out.flush()
    }
...
fun Routing.addResourceFile(path: String, filename: String, contentType: String) {
    get(path) {
        call.log()
        call.respondBytes(contentType = ContentType.parse(contentType), bytes = getResources(filename))
    }
}

private fun ApplicationCall.log() {
    println("call to ${this.request.uri}")
}
